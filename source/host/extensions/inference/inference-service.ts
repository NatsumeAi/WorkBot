...
import { SAND_RUN_PRIVACY_MODE_FALLBACK } from "../../../shared/node/cursor-backend/cursor-inference.js";
...
export function createHostInference(options: HostInferenceOptions) {
  ...
  return {
    ...cursor,
    resolvePrivacyMode() {
      if (routerSettings.getInferenceProvider() !== "cursor") return SAND_RUN_PRIVACY_MODE_FALLBACK;
      return cursor.resolvePrivacyMode();
    },
    createSession(onRequestId: (requestId: string) => void, sessionOptions?: Parameters<typeof cursor.createSession>[1]) {
      ...
      return createProviderPromptSession(provider, "chat") as ReturnType<typeof cursor.createSession>;
    },
    createSummarizationSession(onRequestId: (requestId: string) => void, sessionOptions?: Parameters<NonNullable<typeof cursor.createSummarizationSession>>[1]) {
      ...
      return createProviderPromptSession(provider, "compact") as ReturnType<NonNullable<typeof cursor.createSummarizationSession>>;
    },
  };
}
