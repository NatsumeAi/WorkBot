...
export const GATEWAY_NO_SERVER_MESSAGE_MARKER = "sand no server is configured (no_server)";
...
export function isNoServerConfiguredError(error: unknown): boolean {
  const seen = new Set<object>();
  let current: unknown = error;
  while (current != null && typeof current === "object" && !seen.has(current)) {
    seen.add(current);
    const node = current as { message?: unknown; cause?: unknown };
    if (typeof node.message === "string" && node.message.includes(GATEWAY_NO_SERVER_MESSAGE_MARKER)) return true;
    current = node.cause;
  }
  return typeof error === "string" && error.includes(GATEWAY_NO_SERVER_MESSAGE_MARKER);
}
