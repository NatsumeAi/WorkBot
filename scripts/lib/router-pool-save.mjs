export function nextKeyId(existing) {
  const taken = new Set((Array.isArray(existing) ? existing : []).map((item) => item?.id).filter(Boolean));
  let n = 1;
  while (taken.has(`KEY_${n}`)) n += 1;
  return `KEY_${n}`;
}

export function prepareRouterPoolSave(state) {
