...
test("Router settings use the trusted backend and display recorded inference usage", async () => {
  ...
  assert.match(rendererPatch, /desktop\.agent\.getSelfHostConnection\(\)/);
  assert.match(rendererPatch, /RServerPanel/);
  assert.match(rendererPatch, /id:"server",label:"Server",icon:"server"/);
  assert.match(rendererPatch, /patchOriginalCursorBootGate/);
  assert.match(rendererPatch, /gate:"shell"/);
  assert.match(rendererPatch, /label:"Jump port"/);
  ...
  assert.match(providers, /documentFromPreset\("openrouter"\)/);
  ...
  assert.match(hostRunner, /backgroundSummarizationPropsOverride/);
  assert.match(cursorSession, /routedProvider !== "cursor"/);
  assert.match(cursorSession, /createProviderPromptSession\(routedProvider, sessionOptions\?\.isSummarizationSession === true \? "compact" : "chat"\)/);
  assert.match(cursorBackend, /routedProvider !== "cursor"/);
  assert.match(cursorBackend, /createProviderPromptSession\(routedProvider\)/);
  assert.match(rendererPatch, /desktop\.agent\.setInferenceEndpoints/);
  assert.match(rendererPatch, /apiKeySecret/);
  assert.match(rendererPatch, /gpt-5\.6-sol/);
  assert.match(rendererPatch, /Compress at/);
  assert.match(rendererPatch, /title:"Models"/);
  assert.match(rendererPatch, /title:"Assignments"/);
  assert.match(rendererPatch, /Add model/);
  assert.match(rendererPatch, /label:"Chat"/);
  assert.match(rendererPatch, /label:"Compress"/);
  assert.match(rendererPatch, /Add fallback/);
  assert.match(rendererPatch, /reasoningEffort/);
  assert.doesNotMatch(rendererPatch, /Endpoints JSON/);
  assert.doesNotMatch(rendererPatch, /gpt-4o/);
  assert.doesNotMatch(rendererPatch, /"apiKey":/);
